...
- **Ports / Adapters**：共享契约定义核心端口，Mock、Naive UI、HTTP 等作为适配器
...
| ADR-003 | 选用 Naive UI 作为 Vue3 宿主组件库 | 2026-05-19 | ✅ 已采纳 | 待补全 |
...
**决策**：正式宿主仅保留 `apps/vue3-app`，使用 Vue3 + Naive UI。不包含 React 宿主。  
**后果**：共享包无需跨框架适配，`shared-ui` 仅实现 Vue 组件，`shared-i18n` 仅提供 Vue 初始化，工程维护成本显著降低。

#### ADR-003：选用 Naive UI 作为组件库
**背景**：需要一套企业级 Vue3 组件库，要求类型安全、主题可定制、Tree Shaking 友好。  
**决策**：采用 **Naive UI**，理由包括原生 Vue3 开发体验、细粒度 `themeOverrides`、优秀的 TypeScript 推导、活跃社区。  
**替代方案**：Ant Design Vue（维护节奏及体积考量）、Element Plus（主题定制灵活性稍弱）。  
**后果**：`design-tokens` 导出 Naive UI 主题覆盖对象，`shared-ui` 组件基于 Naive UI 封装，国际化使用 `n-config-provider` + `createLocale`。

#### ADR-004：pnpm catalog 统一版本管理
**背景**：多包 Monorepo 需避免依赖版本碎片化。  
**决策**：在 `pnpm-workspace.yaml` 定义 `catalog`，核心依赖版本统一声明，子包使用 `"naive-ui": "catalog:"` 引用。  
**后果**：单点升级，版本冲突显性化。
...
| Naive UI 停止维护或出现重大不兼容升级 | ADR-003（组件库选型） |
...
├─ packages/
│  ├─ design-tokens/           # 设计令牌（CSS 变量、Naive UI 主题覆盖、UnoCSS 预设）
...
- `shared-service` 不得依赖 `vue`, `vue-router`, `pinia`, `naive-ui`, `@antv/g2`, `bpmn-js`
- `shared-ui` 不得依赖 `apps/*`
...
| `shared-i18n` | vue-i18n (peer), naive-ui | 宿主应用 |
| `shared-service` | shared-utils, axios, msw | UI 框架, DOM |
| `shared-ui` | design-tokens, shared-utils, shared-i18n, shared-service, naive-ui, @antv/g2 | 宿主应用 |
| `shared-workflow` | design-tokens, shared-utils, bpmn-js, naive-ui | 宿主应用业务规则 |
...
#### `design-tokens` – 设计令牌
- **提供**：CSS 自定义属性、Naive UI `GlobalThemeOverrides`、UnoCSS 预设（颜色、字号、间距、圆角等）、图表配色常量
- **子路径**：`./tokens.css`, `./naive-theme`, `./uno-preset`
- **不负责**：业务状态、私有主题逻辑
...
#### `shared-i18n` – 国际化
- **提供**：中英文语言包、`createVueI18n` 初始化函数、Naive UI locale 映射 (`zhCN`, `enUS`)
- **契约**：持久化 key `locale`，运行时切换，通过 `n-config-provider` 联动组件内部文案
...
`shared-ui` 负责提供基于 Naive UI 二次封装的 Vue3 组件。封装遵循以下分层原则：

| 封装模式 | 适用场景 | 示例 |
|----------|----------|------|
| **直接透传** | 组件 Props 与 Naive UI 完全一致，无需额外逻辑 | `n-button` → 直接 re-export |
| **Props 重组织** | 需要统一 API 风格或注入平台语义 | `n-menu` → `SidebarMenu`，接收 `MenuItem[]` 并自动处理权限过滤 |
| **组合封装** | 由多个 Naive UI 组件组装而成 | `PageContainer` = `n-layout-content` + `n-h4` + 共享样式 |

**约束**：
- 所有组件样式通过 `design-tokens` 的 CSS 变量和 Naive UI `themeOverrides` 控制，禁止在组件内硬编码颜色、字号等视觉属性
...
#### `apps/vue3-app` – 组合根
- **技术栈**：Vue3 + Pinia + vue-router + Naive UI + vue-i18n
...
5. 注册 Naive UI (`n-config-provider` 传入主题和语言)
6. 注册 Pinia、Router
7. 挂载 Vue 应用
...
## 8. 国际化方案
- 运行时切换，`n-config-provider :locale` 联动 Naive UI 文案
...
通过 `pnpm-workspace.yaml` 的 `catalog` 统一版本，子包使用 `"naive-ui": "catalog:"` 引用。
...
| 状态管理 | Pinia | ^2.2 |
| 路由 | vue-router | ^4.4 |
| 组件库 | Naive UI | ^2.38（以 catalog 实际锁定版本为准） |
| 国际化 | vue-i18n | ^10 |
| 样式 | UnoCSS | ^0.65 |
| 图表 | AntV G2 | ^5.2 |
| 请求 | Axios | ^1.7 |
...
2. 更新 `design-tokens` 中的主题导出（如从 `naive-theme.ts` 切换为其他组件库主题配置）
3. 更新 `shared-i18n` 中的 locale 映射（如从 Naive UI 的 `zhCN` 切换为其他库的语言包）
...
- §12 Naive UI 版本加注“以 catalog 实际锁定版本为准”
