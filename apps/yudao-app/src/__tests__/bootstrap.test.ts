...
describe('app bootstrap helpers', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('VITE_APP_TITLE', '芋道管理系统')
    vi.stubEnv('DEV', true)
  })
  ...
  it('fails fast when required env VITE_API_BASE_URL is missing', async () => {
  ...
  it('fails fast when required env VITE_APP_TITLE is missing', async () => {
    vi.stubEnv('VITE_APP_TITLE', '')

    const { validateEnv } = await import('../bootstrap/env')

    expect(() => validateEnv()).toThrow(/VITE_APP_TITLE/)
  })
