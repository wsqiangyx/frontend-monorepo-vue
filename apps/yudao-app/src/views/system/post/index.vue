<template>
  <PageContainer title="岗位管理">
      ...
      <t-table :data="tableData" :columns="columns" :loading="loading" row-key="id">
        ...
        <template #action="{ row }">
          <t-space>
            ...
            <t-popconfirm content="确认删除该岗位？" @confirm="handleDelete(row)">
    ...
    <t-dialog v-model:visible="dialogVisible" :header="isEdit ? '编辑岗位' : '新增岗位'" width="500px">
      <t-form :data="formData" label-width="100px">
        ...
        <t-form-item label="岗位编码" name="code">
          <t-input v-model="formData.code" placeholder="请输入岗位编码" />
        </t-form-item>
...
async function fetchData() {
  loading.value = true
  try {
    const res = await fetch('/api/system/post/page?pageNo=1&pageSize=50')
...
function handleAdd() {
  isEdit.value = false
  Object.assign(formData, { id: undefined, name: '', code: '', sort: 0, status: 0, remark: '' })
...
async function handleSubmit() {
  const url = isEdit.value ? '/api/system/post/update' : '/api/system/post/create'
...
async function handleDelete(row: Record<string, unknown>) {
  await fetch(`/api/system/post/delete?id=${row.id}`, { method: 'DELETE' })
