<template>
  <PageContainer title="角色管理">
    ...
    <t-dialog v-model:visible="dialogVisible" :header="isEdit ? '编辑角色' : '新增角色'" width="600px">
      <t-form :data="formData" label-width="100px">
        ...
        <t-form-item label="显示顺序" name="sort">
...
const columns = [
  { colKey: 'id', title: '角色编号', width: 80 },
  { colKey: 'name', title: '角色名称' },
  { colKey: 'code', title: '角色标识' },
  { colKey: 'sort', title: '显示顺序', width: 100 },
...
async function fetchData() {
  loading.value = true
  try {
    const res = await fetch('/api/system/role/page?pageNo=1&pageSize=50')
...
async function handleSubmit() {
  const url = isEdit.value ? '/api/system/role/update' : '/api/system/role/create'
...
async function handleDelete(row: Record<string, unknown>) {
  await fetch(`/api/system/role/delete?id=${row.id}`, { method: 'DELETE' })
