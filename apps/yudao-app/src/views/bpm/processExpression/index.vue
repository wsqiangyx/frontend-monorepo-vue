  ...
  <PageContainer title="流程表达式">
    ...
    <t-dialog v-model:visible="formVisible" :header="formTitle" width="500px" @confirm="handleSubmit">
      <t-form :data="formData" label-width="80px">
        <t-form-item label="表达式名称">
          <t-input v-model="formData.name" placeholder="请输入表达式名称" />
        </t-form-item>
        <t-form-item label="表达式">
          <t-textarea v-model="formData.expression" placeholder="请输入表达式" />
        </t-form-item>
...
const formTitle = ref('新增流程表达式')
const formData = ref<Record<string, unknown>>({ name: '', expression: '', status: 0, remark: '' })
const columns = [
  { colKey: 'id', title: '编号', width: 80 },
  { colKey: 'name', title: '表达式名称' },
...
async function fetchData() {
  loading.value = true
  try {
    const res = await fetch('/api/bpm/process-expression/page?pageNo=1&pageSize=50')
...
function handleAdd() {
  formTitle.value = '新增流程表达式'
  formData.value = { name: '', expression: '', status: 0, remark: '' }
  formVisible.value = true
}

function handleEdit(row: Record<string, unknown>) {
  formTitle.value = '编辑流程表达式'
  formData.value = { ...row }
  formVisible.value = true
}
...
async function handleSubmit() {
  try {
    ...
    const url = formData.value.id ? '/api/bpm/process-expression/update' : '/api/bpm/process-expression/create'
...
async function handleDelete(row: Record<string, unknown>) {
  try { await fetch(`/api/bpm/process-expression/delete?id=${row.id}`, { method: 'DELETE' }); MessagePlugin.success('删除成功'); fetchData() } catch { MessagePlugin.error('删除失败') }
}
